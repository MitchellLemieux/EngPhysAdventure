...
5. When puting a needed or dropped object.name in an interact it MUST be lowercase.
All object keys in the game are stored lowercase and will throw a key error if not defined lowercase.
"""
...
def Reset():
    ...
    # WTF is this doing??? Did I extend this with dim right?
    MAPS1 = [ [ [ [None for dim in range(DRANGE)] for z in range(ZRANGE)] for y in range(YRANGE)] for x in range(XRANGE) ]#0-6,0-8,0-3, 0-1
    ...
    Map("Forest Entrance", (4, 0, 0, 4), "FOREST ENTRANCE: (\S)You can go forward... or back the way you came.",
            "You enter the dense brush full of curiousity. (\S)You feel a cool breeze rush past you as if it is an omen of danger lurking within. (\S)'Be careful in here...' you think to yourself.", ('l', 'r'), False),
    ...
    Map("Forest", (7, 3, 0, 4), "FOREST: (\S)",
        "You get the feeling you cannot go anywhere but back the way you came...", ('f'), False),
    ...
    Map("Forest", (0, 0, 0, 4), "FOREST: (\S)",
        "You get the feeling you cannot go anywhere but back the way you came...", ('r'), False),
    ...
    Map("Forest", (5, 9, 0, 4), "FOREST: (\S)",
        "You get the feeling you cannot go anywhere but back the way you came...", ('b'), False),
    ...
    Map("Forest", (7, 9, 0, 4), "FOREST: (\S)",
        ...
        ('f', 'l'), False),
    Map("Forest", (8, 9, 0, 4), "FOREST: (\S)","Through the brush you can faintly make out what appears to be a cabin... (\S)It makes sense then, why the nearby trees were cut and where they have gone... (\S)You see a 'DO NOT ENTER' sign nailed to a nearby tree. (\S)It might be a good idea to make sure whoever lives there is gone before trying to go inside.", ('f', 'b'), False),
    ...
    Map("Cabin Main Room", (0, 0, 0, 5), "CABIN MAIN ROOM: (\S)","With a long creaking sound the door swings open... (\S)It is very apparent that this place is lived in. (\S)You wonder what else may lie within.",('b', 'f'), False),
    ...
    Equipment("WAW Case", (2, 4, 3,0), "WAWCase.jpg", "Call of Duty World at War for the PS3. The first game to indroduce Zombies Mode. A fine game but defiently not school appropriate. You open the case and the disk isn't in it.","off-hand", (1, 1, 1), ""),
    ...
    Equipment("Pizza Box", (0,3,1,0), "PizzaBox.jpg", "Fully equipped with crusted cheese.", "body", (1,5,1),""),
    ...
    Equipment("2 Sticks", (9, 1, 0, 4), "Stick.jpg", "It is a stick found in the forest.", "hand", (10, 10, 10),-1),
    Equipment("3 Sticks", (0, 3, 0, 4), "Stick.jpg", "It is a stick found in the forest.", "hand", (12, 12, 12),-1),
    Equipment("4 Sticks", (1, 5, 0, 4), "Stick.jpg", "It is a stick found in the forest.", "hand", (14, 14, 14),-1),
    Equipment("5 Sticks", (5, 6, 0, 4), "Stick.jpg", "It is a stick found in the forest.", "hand", (15, 15, 15),-1),
    ...
    Equipment("Heavy Rock", (3, 4, 0, 4), "HeavyRock.jpg", "It is a stick found in the forest.", "hand", (5, 5, 5),-1),
    ...
    Enemy("Dr. Knights","'Whoever took the 3W Textbook from the QT shall feel my eternal wrath...'",(None),(200,100,50),300,"3w textbook","3w textbook","'I've been looking all over for that, have a cold one on me!'\n'But before you go you should know that a difficult road lies ahead.'\n'It will not be easy to have your Iron Ring returned to you'\n'Return the 3W Texbook to the Quantum Tunnel, then Dr. Haugen should be the one you seek next.'","'What are you doing?!'"),
    ...
    Enemy("Bill the MAC Cop","'Give me your student card!'",(3,5,1,0),(50,25,10),50,"solar cell",None,"","We aren't able to arrest you anyway!"),
    ...
    Enemy("Zack the Snack","'I need a snack that smiles back!'",(2,4,0,0),(17,12,15),25,"fanny pack","goldfish","Whoa thats whack! Check out this fanny pack.","Why are you the way you are?"),
    Enemy("Will the MAC Cop","'Give me your student card!'",(4,2,1,0),(50,25,10),50,"fake gun",None,"","We aren't able to arrest you anyway!"),
    Enemy("Andreas the Nerd","'I heard some jebroni took the 3W Textbook and hid in Bridges cafe.'\n'Full send, dude!'",(3,2,1,0),(10,15,5),50,"visor glasses",None,"","Dude, that was definitely a full send"),
    Enemy("Eric the Baller","'Huge talking to people play.'",(0,5,0,0),(20,15,20),50,"Cold Steel Katana",None,"","This is sub-optimal."),
    ...
    Enemy("Nicole the Assistant", "Hi! Welcome to the Engineering Physics Office. What can I help you with? Dr. Lapierre should be just across the tunnel in ABB if you're looking for him.", (1, 5, 2, 0),(15,30,15),40,None,None,"","NO! Did I send you too many emails?"),
    ...
    Enemy("Little Girl", "Hi, I think I'm lost... (\S)Have you seen my mitten?", (0, 0, 0, 4), (10, 10, 10), 10,"ring pop", "old mitten",
              ...
              "AHHH Nooooo, why would you kill a little girl?"),
    Enemy("Hiker","'Make sure you watch out for that crazy guy deeper in the woods...' (\S) 'I am pretty sure he has an axe that controls this forest somehow.' (\S)'He never leaves his cabin without it unless you could somehow get him out of there in a hurry...'",(9,5,0,4),(25,10,10),50,
          "trail mix",None,None,"NOO MY TRAIL MIX! Why would you kill an innocent hiker?"),
    Enemy("Shady Guy", "'Hey, you got any suckers?' (\S)'Something sweet maybe?'", (1, 4, 0, 4), (25, 15,20), 40, "matches", "ring pop", "'Hey thanks!' (\S)'Maybe you can use this to make a fire while you're out here in these creepy woods...'", "'Why would you kill me! I did not want to be killed!'"),
    Animal("Wolf", "It is a wolf.", (9, 3, 0, 4), (15, 15, 50), 20, "Wolf Meat", None, None, "ARRRROOOOOOOOOO!","You would pet the wolf but it's kinda scary. It might bite you!"),
    Animal("Small Squirrel", "It is a small squirrel",(7,4,0,4),(5,5,30),10,"small squirrel meat",None,None,"It didn't put up much of a fight.","You put your hand out and the squirrel climbs up your hand! You play with it for a bit before carying on"),
    Animal("Big Squirrel", "It is a thicc squirrel", (4, 7, 0, 4), (5, 20, 30), 10, "big squirrel meat", None, None, "Thicc boi didn't put up much of a fight...","You put your hand out and the squirrel climbs up your hand! You play with it for a bit before carying on"),
    Enemy("Elderly Man", "'Oh hi there young one!'(\S)'My eyes aren't like they used be...' (\S)'My life spent in the mines may have taken its toll.' (\S)'I have this note here that I would love to be able to read if only I had my glasses...'",(8,0,0,4),(20,10,5),45,"old pickaxe","aged glasses","'Oh thank you!' (\S)'Let me read this note I have here...' (\S)'Equal parts sulfur and charcoal makes a good boom with some high heat.' (\S)'Hmmm whatever that means... Here take this old pick! (\S)'I dont have any use for it.'","'Nooooo! Why would you kill an old man??'"),
    ]
    ...
    Interact("Garbage Can",(2,3,1,0),"It's a garbage can.","You throw the MSP430 in... Yes, you have chosen wisely.","msp430","Vomit"),
    ...
    Interact("Red Book",(4,0,1,0),"It's a guide to living outdoors. (\S)Flipping through the pages you read: (\S)'Hedysarum alpinum also known as the greenberry is characterized by it's lateral veins.' (\S)'If ingested, casues immediate starvation and death.'","","",""),
    Interact("Blue Book",(4,0,1,0),"It's a biology textbook. (\S)Flipping through you read: (\S)'The mitochondria is the powerhouse of the cell.'","","",""),
    ...
    Interact("Rules Sign",(2,3,1,0),"It reads:(\S) (\S)THERE ARE NO RULES","","",""),
        Interact("McMaster Map",(1,2,1,0),
             ...
             ,"Minnick's glasses","Minnick's glasses"),
    Interact("Workbench",(1,7,0,0),"Looks like an old dusty workbench.","",None,""),
    ...
    Interact("Firepit", (9, 4, 0, 4),
                 "It is a firepit. (\S)Perhaps if I wanted some warmth I should get a log in here.",
                 ...
                 "log", "firepit with log"),
    Interact("Firepit with Log", (None),
                 "If only I had some way of starting the fire quickly... (\S)I should scower the forest for a means of starting this thing.",
                 "You add kindling to the firepit. (\S)Now it should start easily with the help of a source.",
                 "kindling", "lit firepit"),
    Interact("Lit Firepit", (None),
                 "You strike a match and throw it inside. (\S)The kindling catches quickly and the after some time the fire roars to life. (\S)After some temporary day-dreaming by the fire you notice embers start to at the base with a yellow-hot glow.",
                 "You throw in the makeshift explosive and run away quickly. (\S)3... 2... 1... you count. (\S)BOOOM (\S)The makeshift explosive goes off brilliantly (\S)In the distance you hear a loud slam followed by a bellowing roar. (\S)'IM COMIN' you hear echo through the woods.","makeshift explosive",None),
    ...
    Interact("Deep Firepit", (3, 8, 0, 4),
                 ...
                 "stick", "charcoal"),
    Interact("Gate of the Forest", (0, 8, 0, 4),
    ...
    Interact("Stone Pedestal", (0, 6, 0, 4),
                 ...
                 "", None, None),
    Interact("Escape Rope", (0, 6, 0, 4), "", "You grab the rope firmly and begin to climb. You climb into the top of the trees. You see Cootes Drive just beside you? You hop down and wonder how that works. At least you're back.", None, None),
