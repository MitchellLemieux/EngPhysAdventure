...
def Reset():
    ...
    Map("MDCL",(1,0,1,0),"~~:\n","After a solid 5 minutes of meditation in the reflection area. You make your next decision feeling refreshed.",['b','r'],True),
    ...
    Map("CCEM",(1,6,0,0),"~~","The Canadian Centre for Electron Microscopy (CCEM) is pretty cool. How are you even allowed in here? A vibration isolated foundation, imaging at the nanoscale, and micromachining. You reach out to touch one of the billion-dollar machines but think better of it. Leave the science to real adults.",('f','b','l','r','d'),True),
    Map("Parking Garage",(0,0,0,0),"~~","The smooth floor down here makes an excellent skate surface.\nRad.\nIf only you learned how to kickflip.",('f','b','l','r','d'),True),
    ...
    Map("Hospital Basement", (0, 1, 0, 0), "~~","The air down here is heavy and there are even fewer windows than the hospital. 'I should get out of here before I see a clicker' you think to yourself.",('f', 'b', 'l', 'r', 'd'), True),
    ...
    Map("2nd Floor W Wing",(0,1,2,0),"~~","The endless number of rooms are mesmerizing.\nYou don't even know which way to go.\nAfter being lost for 2 hours you circle back to where you entered.",('f','l','r','u'),True),
    Map("2nd Floor E Wing", (0, 0, 2, 0), "~~", "You stumble by a windowless room with a cheery grad student working in it. You walk through a pair of doors into an MRI room. How did you even get here? How do you get out?",('b', 'l', 'u'), True),
    ...
    Map("2nd Floor MDCL",(1,0,2,0),"~~","You struggle and find yourself on the outer wall of MDCL.",['f','b','r','u'],True),
    #3rd Level (X,Y,3)
    ...
    Map("3rd Floor BSB", (3, 4, 3,0), "~3RD FLOOR BSB~:\nYou can only go back down the stairs.",
            ...
            "department up here?? After learning about the Oak Ridges Moraine bring yourself back to reality.",
            ('f', 'b', 'l', 'r', 'u'), True),
    Map("3rd Floor ABB: Haugen's Lab", (1, 6, 3, 0), "~~",
            "This new building isn't even complete yet but they've moved good old Haugen's lab here. The Laser lab is pretty darn cool. Walking around here you notice a number of PhD students looking rather shifty. Hmmm... You wonder why that is.",
            ('f', 'b', 'l', 'r', 'u'), True),
    ...
    # --- Hospital: Dimension 6 ---
    Map("Cabin Bathroom", (0, 1, 0, 5), "CABIN BATHROOM: (\S)",
            "Nothing too out of the ordinary in here... (\S)The toilet is a bit old fashio- wait... (\S)Is the toilet paper actually bark? (\S)Clearly we ARE dealing with a madman. (\S)You look around the bathroom... Behind the door... under the sink...",
            ('l', 'f', 'b'), False)
    ]
    ...
    Equipment("Laser Safety Glasses", (1,6,3,0), "LSGlasses.jpg", "Protection from the UV, my dude.", "head", (0,10,5),""),
    ...
    Equipment("Prototype Device", (0, 0, 2, 0), "ErikMasters.jpg","You find a copper box with an Arduino plugged in. Probably used for nuclear SPECT imaging in an MRI. Just a Guess.", "off-hand",(1, 1, 1), ""),
    Equipment("Brainalizer", (None), "EEGCap.jpg", "This Portable SPECT MRI EEG cap has a ton of electrodes and somehow uses nuclear imaging to measure brain signals. Sounds like a lot of acronyms but you can probably use it to read people's thoughts and stats.","head", (1, 1, 20), ""),
    Equipment("Carabiner Key", (0, 1, 0, 0), "CarabinerKey.jpg","A key that seems to belong to a rad doud. 1V is faded one side. How did they loose it if it's on a carabiner?","off-hand", (1, 1, 20), ""),
    Equipment("Fire Extinguisher", (None), "FireExtinguisher.jpg","The red is faded and the sticker pearling. It looks like it was never serviced and probably doesn't work anymore.","hand", (20, 20, -5), ""),
    # --- Aesthetic Objects ---
    ...
    Enemy("Erik the Sk8r","'Check out this tre flip, pretty tight eh? I lost the key to my lab earlier but I'll find it. Also have you seen my drumstick?'",(0,0,0,0),(15,15,40),20,"Erik's frosted tips",'drumstick',"Aww thanks man! Here, you can have this!","Man, that's dumb!",False),
    ...
    Interact("Transmission Electron Microscopes", (1,6,0,0),"'The Titan 80-300 HB is a double aberration-corrected TEM/STEM that can operate at multiple accelerating voltages to allow imaging of a variety of materials, including beam-sensitive samples'. You don't know what that means but apparently it lets you examine things much more closely in this room.", "", None, "", False),
    Interact("MRI", (0, 0, 2, 0), "Nuclear Magnetic Resonance Imaging is used to flip the angular momentum spin of your atoms in a magnetic field.  You put an RF pulse and tip the field one way and then read the return RF pulse. Magnetic slice select gradient method in the Z, another readout gradient in X, and phase encoding gradient in Y. Just read the spectrum of the return magnetic energies in each axis to make an image. Based on the gyromagnetic ratio you can figure out the spectrographic properties of the material. Simple. (\S)All you know is that it reads brains and probably shows how healthy you are.","You put the SPECT prototype device in the MRI. IT whirls and buzzes before assembling itself into a cap. Maybe that was built into the MATLAB code?", "prototype device", "brainalizer", False),
    Interact("VI Door", (0, 0, 2, 0),
                 "This door appears to be locked.",
                 "The key fits in and you unlock the door. You see a room with no windows, dimly lit, with several workstations around.",
                 None, None, True),
